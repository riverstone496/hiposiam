...
from .symhiposiam import SymHipoSiam
...
def get_model(args):    
    if args.model == 'simsiam':
        ...
        model =  SymHipoSiam(get_backbone(args.backbone), angle=args.angle, rotate_times = args.rotate_times, rnn_nonlin=args.rnn_nonlin, remove_rnn=args.remove_rnn, use_aug = args.use_aug, rnn_type=args.rnn_type, random_rotation=args.random_rotation, rnn_norm=args.rnn_norm)
        if args.proj_layers is not None:
            model.projector.set_layers(args.proj_layers)
